...
invested_companies = pd.read_sql_table("invested_companies", conn)
...
for i in range(len(invested_companies["country_listed"])):
    if invested_companies["country_listed"][i] == "United Kingdom": invested_companies["country_extension"][i] = ".L"
    elif invested_companies["country_listed"][i] == "Germany": invested_companies["country_extension"][i] = ".DE"
    elif invested_companies["country_listed"][i] == "France": invested_companies["country_extension"][i] = ".PA"
...
df = yf.download(list(invested_companies["ticker"] + invested_companies["country_extension"]), start = previous_days("M"), end = previous_days("F"))
...
#   5. APPEND PRICES INTO THE TABLE
df.to_sql("prices_companies", conn, # insert df into prices_companies TABLE using the conn CONNECTION.
